...
describe('RoleReveal', () => {
  ...
  it('reveals secret word for non-impostors when clicked', () => {
    ...
    const revealButton = screen.getByText('Press and hold to reveal').closest('button');
  ...
  it('shows waiting message for non-hosts', () => {
    ...
    expect(screen.queryByRole('button', { name: /start drawing!/i })).not.toBeInTheDocument();
    expect(screen.getByText('Waiting for Host to begin...')).toBeInTheDocument();
  });
});
