        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 py-12 bg-stone-900">
            <div className="max-w-lg w-full space-y-8">
                <div className="text-center space-y-4">
                    <h2 className="text-stone-400 font-medium tracking-widest uppercase text-sm">Room Code</h2>
                    ...
                    <p className="text-stone-500 text-sm">Share this code with your friends to let them join!</p>
                </div>
                ...
                <div className="bg-stone-800 rounded-3xl p-6 shadow-xl border border-stone-700">
                    ...
                    <div className="space-y-3 mb-8">
                        {players.map((player) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${player.id === hostId ? 'bg-gradient-to-br from-amber-400 to-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-stone-700 text-stone-300'}`}>
                                        {player.name.charAt(0).toUpperCase()}
                                    </div>
                                    <div>
                                        <span className="font-semibold text-white text-lg">{player.name}</span>
                                        {player.id === myId && <span className="ml-2 text-xs font-bold text-blue-400 uppercase tracking-wider">(You)</span>}
                                    </div>
                                </div>
                                ...
                                {player.id === hostId && (
                                    <div className="flex items-center text-amber-500 gap-1 bg-amber-500/10 px-2 py-1 rounded border border-amber-500/20">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">Host</span>
                                    </div>
                                )}
                            </div>
                        ))}
                        ...
                        {players.length < 3 && (
                            <div className="p-4 rounded-xl border border-dashed border-stone-600 bg-stone-800/50 text-center flex flex-col items-center gap-2">
                                ...
                                <p className="text-xs text-stone-500">Need at least 3 players to start</p>
                            </div>
                        )}
                    </div>
                    ...
                    {isHost ? (
                        ...
                        <div className="text-center p-4 bg-stone-900 rounded-xl border border-stone-700 animate-pulse">
                            <span className="text-stone-400 font-medium">Waiting for host to start...</span>
                        </div>
                    )}
                </div>
            </div>
