...
export const GameResult: React.FC = () => {
    ...
    const impostorName = players.find(p => p.id === impostorId)?.name || 'Unknown';
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-stone-950">
            ...
            <div className="max-w-2xl w-full text-center space-y-8 z-10">
                ...
                <div className={`p-8 rounded-3xl border-2 ${impostorCaught ? 'bg-emerald-950/40 border-emerald-500/30' : 'bg-red-950/40 border-red-500/30'}`}>
                    ...
                    <div className="text-xl md:text-2xl text-stone-300 font-medium space-y-2">
                        {!ejectedResult ? (
                            ...
                            <p><span className="font-bold text-white">{ejectedName}</span> was ejected.</p>
                        )}
